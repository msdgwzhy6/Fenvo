...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    //微博的内容的高度
    CGFloat wbDetailWidth = self.view.frame.size.width - WBStatusCellControlSpacing * 2;
    ...
    yHeight += wbDetailSize.height + WBStatusCellControlSpacing;
    ...
    if (weiboMsg.retweeted_status != nil)
    {
        ...
        //转发内容的文本内容
        NSString *retwitterContentText = [NSString
                                          stringWithFormat:@"%@：%@",
                                          weiboMsg.retweeted_status.user.screen_name,
                                          weiboMsg.retweeted_status.wbDetail];
        CGSize retweeted_wbDetailSize = [retwitterContentText
                                         boundingRectWithSize:CGSizeMake(wbDetailWidth, MAXFLOAT)
                                         options:NSStringDrawingUsesLineFragmentOrigin
                                         attributes:@{NSFontAttributeName:WBStatusCellDetailFont}
                                         context:nil].size;
        yHeight += retweeted_wbDetailSize.height + WBStatusCellControlSpacing;
        
        //那么图像就在转发部分进行显示
        if (weiboMsg.retweeted_status.thumbnail_pic) //转发的微博有图像
        {
            if (weiboMsg.retweeted_status.pic_urls.count == 1) {
                yHeight += (120 + WBStatusCellControlSpacing);
            }
            else{
                if(weiboMsg.retweeted_status.pic_urls.count >6){
                    yHeight += (180 + WBStatusCellControlSpacing*3);
                }
                else if (weiboMsg.retweeted_status.pic_urls.count <= 3){
                    yHeight += (60 + WBStatusCellControlSpacing);
                }else{
                    yHeight += (120 + WBStatusCellControlSpacing*2);
                }
            }
        }
    }
    ...
    else
    {
        ...
        if (weiboMsg.thumbnail_pic) {
            if (weiboMsg.pic_urls.count == 1) {
                ...
                if (weiboMsg.pic_urls.count > 6) {
                    yHeight += (180 + WBStatusCellControlSpacing*3);
                }else if (weiboMsg.pic_urls.count <= 3){
                    yHeight += (60 + WBStatusCellControlSpacing);
                }
                else{
                    yHeight += (120+ WBStatusCellControlSpacing*2);
