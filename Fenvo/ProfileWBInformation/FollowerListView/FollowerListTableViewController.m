...
#define WBAPIURL_FOLLOWER @"https://api.weibo.com/2/friendships/followers.json"

@interface FollowerListTableViewController()<UIScrollViewDelegate>
{
...
- (FollowerListViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    // 0.用static修饰的局部变量，只会初始化一次
    static NSString *ID = @"followerCell";//注意cell中，它的identifier设置为“cell”
    
    // 1.拿到一个标识先去缓存池中查找对应的Cell
    FollowerListViewCell *cell;
        
    cell = (FollowerListViewCell *)[tableView dequeueReusableCellWithIdentifier:ID];
    if (cell == nil) {
        UINib *nibs = [UINib nibWithNibName:@"FollowerListViewCell" bundle:nil];
