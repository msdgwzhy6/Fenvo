...
@property (strong) WeiboTextStorage *textStorage;
...
@property (nonatomic, strong) NSDictionary *attributesAccount;
...
- (void)determineHotWords {
    ...
    _textStorage = [[WeiboTextStorage alloc] init];
    ...
    // Define a character set for hot characters (@ account, # hashtag)
    ...
    while ([tmpText rangeOfCharacterFromSet:hotCharactersSet].location < tmpText.length) {
        ...
        WeiboHotWord hotWord;

        switch ([tmpText characterAtIndex:range.location]) {
            case '@':
                hotWord = WeiboAccount;
                break;
            case '#':
                hotWord = WeiboHashtag;
                break;
            default:
                break;
        }
...
- (void)determineLinks {
    ...
    [regex enumerateMatchesInString:tmpText options:0 range:NSMakeRange(0, tmpText.length) usingBlock:^(NSTextCheckingResult *result, NSMatchingFlags flags, BOOL *stop) {
        ...
        if ([_validProtocols containsObject:protocol.lowercaseString]) {
            [_rangesOfHotWords addObject:@{@"hotWord": @(WeiboLink), @"protocol": protocol, @"range": [NSValue valueWithRange:result.range]}];
        }
    }];
}
...
- (void)updateText
{
    ...
    for (NSDictionary *dictionary in _rangesOfHotWords)  {
        ...
        WeiboHotWord hotWord = (WeiboHotWord)[[dictionary objectForKey:@"hotWord"] intValue];
...
- (void)setDetectionBlock:(void (^)(WeiboHotWord, NSString *, NSString *, NSRange))detectionBlock {
    if (detectionBlock) {
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    if(touchedHotword != nil) {
        ...
        _detectionBlock((WeiboHotWord)[[touchedHotword objectForKey:@"hotWord"] intValue], [_cleanText substringWithRange:range], [touchedHotword objectForKey:@"protocol"], range);
    } else {
